...
## 4. Local Flag Integration

- [ ] 4.1 Add `--local` flag to `list-red-flags` command, routing to SQLiteStorage.get_cards_by_flag(1) when set
- [ ] 4.2 Add `--local` flag to `get-examples-for-red-flags-cards` command
- [ ] 4.3 Add stale data warning when get_last_sync() is older than 24 hours
- [ ] 4.4 Add "no cached data" warning when `--local` is used and get_last_sync() returns None

## 5. Tests

- [ ] 5.1 Unit tests for `SQLiteStorage` using in-memory SQLite (init, replace_all, upsert_notes, queries, sync_log, atomicity on failure)
- [ ] 5.2 Unit tests for upsert_notes: mix of new/existing, all new, all existing, empty input
- [ ] 5.3 Unit tests for sync_log: add entry, get_last_sync, epoch/ISO timestamp format
- [ ] 5.4 Unit tests for data transformation: AnkiConnect response → notes row format
- [ ] 5.5 Unit tests for sync mode detection: first sync → initial, subsequent → incremental
- [ ] 5.6 Integration test for initial sync command (mocked AnkiConnect, real SQLite)
- [ ] 5.7 Integration test for incremental sync command (mocked AnkiConnect, pre-populated SQLite)
- [ ] 5.8 Integration tests for `--local` flag on `list-red-flags` and `get-examples-for-red-flags-cards`
- [ ] 5.9 Unit tests for new AnkiConnect methods (get_all_note_ids, get_all_card_ids, get_notes_info, get_cards_info, find_notes_modified_since)
